...
const LecturerHallAvailability = () => {
  ...
  const handleBook = async (hallId) => {
    ...
    try {
      ...
      await lecturerService.bookHall({ hallId, date, startTime, endTime, subject, targetBatch, capacity });

      toast.success("Booking request sent to HOD successfully!");
      handleSearch();
    } catch (err) {
      const status = err?.response?.status;
      if (status === 401) {
      ...
      <div className="space-y-10">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-8 space-y-6 transition-colors">
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={handleClear}
              className="text-sm px-4 py-2 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-200 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
            >
              Clear Filters
            </button>
          </div>
          ...
          <div className="grid md:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <input
                ...
                className="mt-1 w-full border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 rounded-xl px-4 py-2 focus:ring-2 focus:ring-indigo-500 transition-colors"
              />
            </div>
                ...
                <div
                  ...
                  className={`rounded-2xl border p-6 transition hover:shadow-md ${
                    hall.status === "AVAILABLE"
                      ...
                      : "bg-gray-100 dark:bg-slate-900/60 border-gray-200 dark:border-slate-700 opacity-70"
                  }`}
                >
                  ...
                  <div className="mt-6">
                    {hall.status === "AVAILABLE" ? (
                      ...
                      <button
                        disabled
                        className="w-full bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300 py-2 rounded-lg cursor-not-allowed"
                      >
                        Pending Request
                      </button>
                    ) : (
                      <button
                        disabled
                        className="w-full bg-gray-300 dark:bg-slate-700 text-gray-600 dark:text-slate-300 py-2 rounded-lg cursor-not-allowed"
                      >
                        Not Available
                      </button>
                    )}
                  ...
                  {(hall.status === "PENDING" || hall.status === "BOOKED") && (
                    <div className="mt-3 text-sm text-gray-600 dark:text-slate-400">
                      {hall.bookedSlots && hall.bookedSlots.length > 0 && (
                        <div className="text-red-600 dark:text-red-300">Booked: {hall.bookedSlots.join(", ")}</div>
                      )}
                      {hall.lockedSlots && hall.lockedSlots.length > 0 && (
                        <div className="text-yellow-700 dark:text-yellow-300">Pending: {hall.lockedSlots.join(", ")}</div>
                      )}
                    </div>
                  )}
                </div>
